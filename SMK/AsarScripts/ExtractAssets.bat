...
echo - Graphics: SMK (USA)/ SMK (USA Rev. 1)
echo - Samples: SMK (USA)/ SMK (USA Rev. 1)
echo - Music: SMK (USA)/ SMK (USA Rev. 1)
echo.

:Start
echo Place a headerless SMK ROM named %ROMName% in this folder, then type the number representing what version %ROMName% is.
echo 0 = SMK (USA)/ SMK (USA Rev. 1)
echo 1 = SMK (Japan)
echo 2 = SMK (PAL)
...
echo You need to place a SMK ROM named %ROMName% in this folder before you can extract any assets^^!
goto :Mode
...
if %PointerSet% equ 18 goto :MoveMusic
if %PointerSet% equ 24 goto :MoveBRR
goto :MoveNothing
...
if not exist "07.brr" goto :NoSplitBRR
if not exist "07_02.brr" goto :NoSplitBRR
echo: Merging 07.brr and 07_02.brr

echo:%MemMap% >> TEMP1.asm
echo:^^!Offset #= filesize("07.brr") >> TEMP1.asm
echo:org $C00000+^^!Offset >> TEMP1.asm
echo:incbin 07_02.brr >> TEMP1.asm
asar --fix-checksum=off --no-title-check "TEMP1.asm" "07.brr"
if exist TEMP1.asm del TEMP1.asm
if exist 07_02.brr del 07_02.brr
:NoSplitBRR
